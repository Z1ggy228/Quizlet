...
export default function Learn({ cards, setName, onMastery, onExit }) {
  ...
  /**
   * Пишем ответ в базу: mastery, расписание SM-2 и счётчики показов/ошибок.
   * Записанные значения кладём обратно в свою копию карточки — иначе следующий
   * ответ по этому слову посчитается от устаревших ease_factor и interval_days.
   */
  async function saveAnswer(st, opts) {
    try {
      const patch = await db.recordAnswer(st.card, opts)
      const merge = (list) =>
        list.map((s) => (s.id === st.id ? { ...s, card: { ...s.card, ...patch } } : s))
      nextStatesRef.current = merge(nextStatesRef.current)
      setStates(merge)
    } catch (e) {
      setError('Прогресс не сохранился: ' + e.message)
    }
  }
  ...
  function answer(given) {
    ...
    const quality = qualityOf({ correct, type: question.type })

    if (correct) playCorrect()
    else playWrong()

    // Прогресс уходит в базу сразу, чтобы он не потерялся при закрытии вкладки.
    saveAnswer(st, { mastery, quality, correct })
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="relative mt-5 overflow-hidden p-6">
        {feedback?.correct && (
          <span
            className="pointer-events-none absolute inset-x-0 top-6 z-10 animate-float-up text-center
              font-display text-3xl text-emerald-500 drop-shadow-[0_2px_8px_rgba(255,255,255,0.9)]
              dark:drop-shadow-[0_2px_8px_rgba(2,6,23,0.9)] sm:text-4xl"
          >
            {feedback.praise}
          </span>
        )}
      ...
      <Modal open={settingsOpen} onClose={() => setSettingsOpen(false)} title="Настройки Learn">
        <div className="space-y-4">
          ...
          {soundSupported && (
            <label className="flex cursor-pointer items-center justify-between gap-3 rounded-lg p-1">
              <span className="text-sm font-medium">Звуки ответов</span>
              <input
                type="checkbox"
                checked={sound}
                onChange={(e) => {
                  ...
                  if (e.target.checked) playCorrect() // сразу слышно, что включилось
                }}
...
function FinalScreen({ total, mastered, reviewed, stats, onExit, onRestart, resetting, error }) {
  ...
  // Фанфар один раз, при появлении экрана.
  useEffect(() => {
    playFinish()
  }, [])
